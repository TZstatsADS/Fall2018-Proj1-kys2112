...
packages.used=c("tm", "tidytext", "tidyverse", "wordcloud",
                "ggplot2", "dplyr", "wordcloud", "topicmodels", 
                "ngram", "quanteda", "reshape2")
...
hm_top_terms %>%
  ...
  coord_flip() +
  ggtitle("Topic Model for Silent Generation")
```
...
hm_top_terms %>%
  ...
  facet_wrap(~ topic, scales = "free") +
  coord_flip() +
  ggtitle("Topic Model for Generation Z")
```
...
bag_of_words %>%
  ...
  ggtitle("Gen-Z/Silent Log Ratio")
```
...
hm_data %>%
  ...
  coord_flip() +
  ggtitle("Gen-Z and Silent Gen Bigrams Comparison")
```
...
bag_of_words %>%
  ...
  comparison.cloud(colors = c("red", "blue"), max.words = 40)
```
...
hm_data %>%
  ...
  ggtitle("Average Sentence Length for Gen-Z and Silent Gen")
```
